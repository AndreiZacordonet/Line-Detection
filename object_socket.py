...
class ObjectSenderSocket:
    ...
    def await_receiver_connection(self):
        ...
        if self.print_when_awaiting_receiver:
            print(f'[{datetime.datetime.now()}][ObjectSenderSocket/{self.ip}:{self.port}] \
            awaiting receiver connection...')
    ...
    def send_object(self, obj: Any):
        ...
        if self.print_when_sending_object:
            print(f'[{datetime.datetime.now()}][ObjectSenderSocket/{self.ip}:{self.port}] \
            Sent object of size {data_size} bytes.')
...
class ObjectReceiverSocket:
    ...
    def recv_object(self) -> Any:
        ...
        if self.print_when_receiving_object:
            print(f'[{datetime.datetime.now()}][ObjectReceiverSocket/{self.ip}:{self.port}] \
            Received object of size {obj_size_bytes} bytes.')
        return obj
